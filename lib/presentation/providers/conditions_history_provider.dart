...
class ConditionsHistoryState {
  ...
  const ConditionsHistoryState({required this.currentPage});

  ConditionsHistoryState copyWith({int? currentPage}) {
    return ConditionsHistoryState(currentPage: currentPage ?? this.currentPage);
  }
}

/// State notifier for conditions history
class ConditionsHistoryNotifier extends StateNotifier<ConditionsHistoryState> {
  ConditionsHistoryNotifier()
    : super(const ConditionsHistoryState(currentPage: 1));
